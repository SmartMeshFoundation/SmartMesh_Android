...
public class LineChartView extends View {
    ...
    private Paint pointPaintCircle;//曲线上锚点画笔
    ...
    private int rulerValueDefault = 100;
    ...
    private int lineColor = Color.parseColor("#3F7AE0");//曲线颜色
    private float lineWidthDP = 1.5f;//曲线宽度dp

    private int pointColor = Color.parseColor("#FFFFFF");//锚点颜色
    private int pointColorCircle = Color.parseColor("#3F7AE0");//锚点颜色
    private float pointWidthDefault = 3.5f;
    private float pointWidthDP = pointWidthDefault;//锚点宽度dp

    private int tableColor = Color.parseColor("#3F7AE0");//表格线颜色
    private int rulerTextColor = Color.parseColor("#000000");//表格文本颜色

    private float tableWidthDP = 0.5f;//表格线宽度dp
    private float rulerTextSizeSP = 10f;//表格标尺文本大小

    private int pointTextColor = Color.parseColor("#3F7AE0");//锚点文本颜色
    ...
    private void setupView() {
        ...
        pointPaintCircle = new Paint();
        pointPaintCircle.setAntiAlias(true);
        pointPaintCircle.setStyle(Paint.Style.STROKE);
        pointPaintCircle.setColor(pointColorCircle);
        pointPaintCircle.setStrokeWidth(dip2px(pointWidthDP));
        ...
        textRulerPaint.setStyle(Paint.Style.STROKE);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        rulerValueDefault = ((int)((maxValue/5)/100))*100;
        if (rulerValueDefault <= 0){
            rulerValueDefault = 100;
        }
        rulerValue = rulerValueDefault;
        if (!isInitialized) {
            setupLine();
        }
    ...
    private int getTableStart() {
        return isShowTable ? stepStart + tablePadding - 10 : stepStart;
    }
    ...
    private void drawTable(Canvas canvas) {
        ...
        tablePath.moveTo(stepStart, - getValueHeight(rulerMax) + 50);//加上顶部的间隔
        ...
        do {
            ...
            startValue = startValue + rulerValueDefault;
    ...
    private void drawRulerXValue(Canvas canvas) {
        ...
        for (int i = 0; i < linePoints.length; i++) {
            ...
            if (point == null) break;
            drawRulerXText(canvas, dataList.get(i).getX(), linePoints[i].x, 0);
        }
    }
    ...
    private void drawLinePoints(Canvas canvas) {
        ...
        for (int i = 0; i < pointCount; i++) {
            Point point = linePoints[i];
            if (point == null) break;
            canvas.drawCircle(point.x, point.y, pointWidth * 1.05f, pointPaintCircle);
            canvas.drawCircle(point.x, point.y, pointWidth , pointPaint);
            //绘制点的文本
            drawLinePointText(canvas, String.valueOf(dataList.get(i).getY()), point.x, point.y);
        }
    }
    ...
    private void setupLine() {
        ...
        Point pre = new Point();
        pre.set(stepTemp, -getValueHeight((int) dataList.get(0).getY()));//坐标系从0,0默认在第四象限绘制
        linePoints[0] = pre;
        ...
        for (int i = 1; i < dataList.size(); i++) {
            Entry data = dataList.get(i);
            Point next = new Point();
            next.set(stepTemp += stepSpace, -getValueHeight((int) data.getY()));
    ...
    public void setData(List<Entry> dataList) {
        ...
        maxValue = (int) Collections.max(this.dataList, (o1, o2) -> (int) (o1.getY() - o2.getY())).getY();

        minValue = (int) Collections.min(this.dataList, (o1, o2) -> (int) (o1.getY() - o2.getY())).getY();
