...
public class IndexScroller{
	...
	public IndexScroller(Context context, ListView lv, ContactsSectionIndexer indexer){
		this(context, lv);
		mIndexer = indexer;
	}
	...
	public void drawSections(Canvas canvas){
		...
		for (int i = 0; i < mSections.length; i++){
			...
			canvas.drawText(mSections[i], mIndexbarRect.left + paddingLeft,mIndexbarRect.top + mIndexbarMargin + sectionHeight * i
	...
	public boolean onTouchEvent(MotionEvent ev){
		switch (ev.getAction()){
			case MotionEvent.ACTION_DOWN:
				// Hung capture the event for later
				lastTouchDownEventX = ev.getX();
				...
				if (mState != STATE_HIDDEN && contains(ev.getX(), ev.getY())){
					...
					}else if(mIndexer.hasSection(mCurrentSection)){
						currentPosition=mIndexer.getPositionForSection(mCurrentSection)+mListView.getHeaderViewsCount()+startIndex;
						mListView.setSelectionFromTop(currentPosition,0);
					}else{
	...
	public void onSizeChanged(int w, int h, int oldw, int oldh){
		mListViewWidth = w;
		mListViewHeight = h;
		mIndexbarRect = new RectF(w - mIndexbarWidth,mIndexbarMargin, w, h - mIndexbarMargin);
	}
	...
	public void setAdapter(Adapter adapter){
		if (adapter instanceof SectionIndexer){
			mIndexer = (ContactsSectionIndexer) adapter;
			mSections = (String[]) mIndexer.getSections();
		} else if (adapter instanceof NewContactListAdapter){
			NewContactListAdapter c = (NewContactListAdapter) adapter;
